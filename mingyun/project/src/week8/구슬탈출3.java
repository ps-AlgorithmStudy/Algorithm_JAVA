...
public class 구슬탈출3 {
    ...
    static void dfs(char[][] map, List<Ball> ballList, int cnt, ArrayDeque<Integer> arrayDeque) {
        ...
        f:for (int d=0;d<4;d++) {
            ...
            if (!(newBallList.get(0).out && newBallList.get(1).out)) {
                ...
                if (isMove) {
                    ...
                    dfs(map,newBallList, cnt+1, arrayDeque);
    ...
    public static void main(String[] args) throws Exception {
        ...
        dfs(map, ballList, 0, new ArrayDeque<>());

        char[] print = {'U','D','L','R'};
        if (result != Integer.MAX_VALUE) {
            System.out.println(result);
            for (int a:resultArr) {
                System.out.printf("%c", print[a]);
            }
            System.out.println();
        }
